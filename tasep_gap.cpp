...
int main() {
  ...
  for (size_t t = 0; t < Brn; t++) {
    ...
    mean = vnew.sum()/vnew.curr_size_;
    for (size_t ii=0; ii<vnew.curr_size_; ii++) vnew[ii].val -= mean;
    end = clock();
    ...
    printf("burn: %ld\t lambda: %lf\t nonzeros: %ld\t time / iteration: %lf\n",
       t+1, pow(N,1.5)*(lambda-1.0)/alpha, v.curr_size_, ((double)end - start)/CLOCKS_PER_SEC);
  }
  ...
  for (size_t t = 0; t < Nit; t++){
    ...
    printf("iteration: %ld\t lambda: %lf\t average: %lf\t nonzeros: %ld\t time / iteration: %lf\n",
       t+1, pow(N,1.5)*(lambda-1.0)/alpha, pow(N,1.5)*(lambda_ave-1.0)/alpha, v.curr_size_, ((double)end - start)/CLOCKS_PER_SEC);
  }
