...
for d = 1:2
    ...
    for m = 1:(nitr-1)
        ...
        RAT = RA[2:n-1,2:n-1]*TRich[2:n-1]
        RAQ = RA[2:n-1,2:n-1]*QRich[2:n-1]
        RAW = RA[2:n-1,2:n-1]*W[2:n-1]

        TRich[2:n-1] = TRich[2:n-1] .- alpha .* RAT
        QRich[2:n-1] = QRich[2:n-1] .- alpha .* RAQ
        W[2:n-1] = W[2:n-1] .- alpha .* RAW
        ...
        TRich[2:n-1] = TRich[2:n-1] .+ alpha .* rhsRT[2:n-1]
        QRich[2:n-1] = QRich[2:n-1] .+ alpha .* rhsRQ[2:n-1]

        resT = rT[2:n-1] .- A[2:n-1,2:n-1]*TRich[2:n-1]
        resQ = rQ[2:n-1] .- A[2:n-1,2:n-1]*QRich[2:n-1]

        cT = dot(W[2:n-1], resT)
        cQ = dot(W[2:n-1], resQ)
        z = dot(W[2:n-1],A[2:n-1,2:n-1]*W[2:n-1])

        TSI[2:n-1] = TRich[2:n-1] .+ (cT/z).*W[2:n-1]
        QSI[2:n-1] = QRich[2:n-1] .+ (cQ/z).*W[2:n-1]

        eTRich[m] = norm(TRich[2:n-1]-T[2:n-1])
        eTSI[m] = norm( TSI[2:n-1] - T[2:n-1] )

        eQRich[m] = norm(QRich[2:n-1]-Q[2:n-1])
        eQSI[m] = norm( QSI[2:n-1] - Q[2:n-1] )
    ...
    plt1 = plot!(plt1, (1:nitr-1), eTRich, lc=Int(2^(d-1)), lw=4, legend=:none)
    plt1 = plot!(plt1, (1:nitr-1), eTSI, lc=Int(2^(d-1)), ls=:dot, lw=4, legend=:none)
